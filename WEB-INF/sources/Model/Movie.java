...
//			ArrayList<Movie> movieList = new ArrayList<>();
			...
			if(rs.isBeforeFirst()){
					...
					String trailer = rs.getString("trailer_url");
                                        squery = "SELECT first_name, last_name, id FROM stars JOIN stars_in_movies ON stars.id = stars_in_movies.star_id WHERE stars_in_movies.movie_id = ?" ;
...
//			Statement statement2 = dbcon.createStatement();
			...
			if(type.equals("title")){
				...
				query = " SELECT * FROM movies WHERE movies.id in ( SELECT movie_id FROM genres join genres_in_movies ON genres.id = genres_in_movies.genre_id WHERE genres.name = ?) ORDER BY " + by + " "  + sort + " LIMIT " + limit + " OFFSET " + offset;
				stmt = dbcon.prepareStatement(query);
				stmt.setString(1, arg);
				rs = stmt.executeQuery();
				...
				query = "SELECT * FROM movies WHERE title like ?  ORDER BY " + by + " "  + sort + " LIMIT " + limit + " OFFSET " + offset;
				String title = arg + "%";
				stmt = dbcon.prepareStatement(query);
